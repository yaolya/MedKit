...
from backend.models import Medicament, Disease, Category, Manufacturer, MedicamentsUsers, Product, User
...
def add_medicament():
    ...
    try:
        ...
        m = db.session.query(Manufacturer).filter_by(name=manufacturer_name).first()
            # d = db.session.query(Disease).filter_by(name=disease_name).first()
        ...
        if manufacturer_name != '':
            ...
            new_medicament = Medicament(name=med_name, image_data=None, image_name=None, pharm_properties=med_pharm_properties, manufacturer_id=m.id, contraindications=med_contraindications, side_effects=med_side_effects, mode_of_application=med_mode_of_application, driving=med_driving, storage_temperature=med_storage_temperature, prescription_required=med_prescription_required)
                # m.medicament = new_medicament
                
            # if disease_name != '':
            #     if d is None:
            #         d = Disease(name=disease_name)
            #         db.session.add(d)
            #     new_medicament.diseases.append(d)
...
def get_image(id):
    ...
    if medicament.image_data is not None:
        return send_file(io.BytesIO(medicament.image_data), attachment_filename=medicament.image_name, as_attachment=True) # attachment_filename=medicament.image_name
    else:
        return ('', 204)
...
def get_medicaments():
    ...
    page = int(request.args['_page'])
    # medicaments_list_json = jsonify(medicaments_list[(page-1)*limit:page*limit])
...
def delete_medicament(id):
    ...
    for product in products:
        ...
        db.session.flush()
    medicaments_users = MedicamentsUsers.query.filter_by(medicament_id=id).all()
